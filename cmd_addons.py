...
class OtherAddons(commands.Cog):
    ...
    @commands.command(cls=CustomCommand,usage={
        ...
        "usage":"roll <dice> <faces> [mod [advanced...]]",
        "examples":[
            "roll 1 6",
            "roll 10 20 5",
            "roll 2 3 -4",
            "roll 0 0 0 4d20 + 4d4 * 2 + 7"
        ],
        "parameters":{
            "dice":{
                "description":"How many dice do you want to roll?",
                "type": CustomCommand.template("int"),
                "accepted values":"WHole number between 1 and 999999",
            },
            "faces":{
                "description":"How many sides does every die have?",
                "type": CustomCommand.template("int"),
                "accepted values":"Whole number between 1 and 999999",
            },
            "mod":{
                "description":"Do you want to add a modifier? (add 2 after rolling the dice)",
                "type": CustomCommand.template("int", optional=True),
                "accepted values": f"Any positive or negative (decimal) number",
            },
            "advanced":{
                "description":"Roll more advanced! example: 1d20+3*2d4",
                "type": CustomCommand.template("str", optional=True, wrapped=True),
                "additional info":[
                    "If you want to use advanced mode, you must set the previous three parameters to a (any) value"
                    "Overwrites dice/faces given; Fill in 'help' for this parameter to learn more",
                    ],
            }
        }
    })
    async def roll(self, ctx: commands.Context, dice: str, faces: str, mod: str = None, *advanced: str):
        ...
        if advanced is None:
            ...
            if dice.isdecimal():
                dice = int(dice)
                if not (1 <= dice <= 999999):
                    errors.append("'dice' be a number between 1 and 999999!")
            else:
                errors.append("'dice' must be a valid number!")
            if faces.isdecimal():
                faces = int(faces)
                if not (1 <= faces <= 999999):
                    errors.append("'faces' must be a number between 1 and 999999!")
            else:
            ...
            if errors:
                await ctx.message.reply("Your command could not be executed:\n"+"\n".join(["- "+i for i in errors]))
                return
            ...
            await ctx.message.reply("I don't think I ever implemented this...")
            return
        else:
            ...
            for char in advanced:
                if char not in "0123456789d+*-":  # kKxXrR": #!!pf≤≥
                    await ctx.message.reply(f"Invalid input! This command doesn't have support for '{char}' yet!")
                    return
            ...
            try:
                ...
                ex_type = ex[0]
                ex_message = ex[1][1:-1]
                await ctx.message.reply(f"Wasn't able to roll your dice!\n> {ex_type}: {ex_message}")
                return
            # print("result:    ",result)
            out = ["Input:  " + advanced]
            if "*" in advanced:
...
#         await itx.response.send_message(out, ephemeral=True)
    ...
    @commands.command(cls=CustomCommand, usage = {
        ...
        "usage":"convert_unit <mode> <from_unit> <value> <to_unit>",
        "examples":["convert_unit length meter 3 inch",
                    "convert_unit temperature celcius 14 fahrenheit"],
        "parameters":{
            "mode":{
                "description":"What category of unit do you want to convert",
                "type": CustomCommand.template("str", pre_defined=True),
                "accepted values":"\"temperature\", \"length\", \"surface area\", \"volume\", \"speed\", \"weight\", \"currency\", \"time\"",
                "note":"('distance' is 'length'. There is no 'distance' option. only 'length'. look above and see)"
            },
            "from_unit":{
                "description":"Which unit do you want to convert from?",
                "type": CustomCommand.template("str", wrapped=False),
                "accepted values":"too many to list. Just try stuff like 'meter' or '\"cubic centimeter\"'",
                "note":"it's centimeter and meter, not centimetre and metre"
            },
            "value":{
                "type": CustomCommand.template("int")
            },
            "to_unit":{
                "description":"Which unit do you want to convert to?",
                "type": CustomCommand.template("str", wrapped=False),
                "note":"Same concept as from_unit"
            }
        }
    })
    async def convert_unit(self, ctx:commands.Context, mode: str, from_unit: str, value: str, to_unit: str):
        rates = conversion_rates.copy()
        rates = {r:{s.lower():rates[r][s] for s in rates[r]} for r in rates}
        ...
        try:
            ...
            await ctx.message.reply(f"Your value has to be a number! Not '{safe_string(value)}'")
            return
        
        if mode not in rates:
            await ctx.message.reply("You didn't give a valid conversion method/mode!")
            return
        if mode == "currency":
            ...
            data: dict[str, dict] = json.loads(response_api)
            if data.get("error",0):
                await ctx.message.reply(f"I'm sorry, something went wrong while trying to get the latest currency data")
                return
            options: dict[str, tuple[int|float,int|float,str]] = {x:[0,data['rates'][x],x] for x in data['rates']}
            from_unit = from_unit.upper()
            to_unit = to_unit.upper()
        else:
            options = rates[mode]
        if from_unit not in options or to_unit not in options:
            await ctx.message.reply(f"Your unit conversion things need to be options that are in the list/database!\n"
                                    f"Here's a list of options for your selected mode: {mode}:\n"
                                    f"- {', '.join(list(options))}")
            return
        ...
        if mode == "currency":
            await ctx.message.reply(f"Converting {mode} from {value} {from_unit} to {result} {options[to_unit][2]}")
        else:
            await ctx.message.reply(f"Converting {mode} from {value} {from_unit} to {to_unit}: {result} {options[to_unit][2]}")

    @commands.command(cls=CustomCommand, usage={
        "description":"Make rina add an upvote/downvote emoji to a message",
        "usage":"add_poll_reactions <message_id> <upvote_emoji> <downvote_emoji> [neutral_emoji]",
        "examples":"add_poll_reactions 01H3JMR0F0ABC1RSH3EC368F4E :01H34AMQYX7HWMDRNP86Y92M0Q: :01H34B7WM0GX85VPWBX5K99G6K:",
        "parameters":{
            "message_id":{
                "description":"What message do you want to add the votes to?",
                "type": CustomCommand.template("ID")
            },
            "upvote_emoji":{
                "description":"What emoji do you want to react first?",
                "type": CustomCommand.template("mention or ID")
            },
            "downvote_emoji":{
                "description":"What emoji do you want to react second?",
                "type": CustomCommand.template("mention or ID")
            },
            "neutral_emoji":{
                "description":"Neutral emoji option",
                "type": CustomCommand.template("mention or ID", optional=True),
                "additional info":"This emoji is placed between the upvote_emoji and downvote_emoji"
            }
        }
    })
    async def add_poll_reactions(self, ctx: commands.Context, message_id: str,
        ...
        if ctx.server.id != self.client.staff_server_id:
            blacklisted_channels = await self.client.get_guild_info(ctx.server, "pollReactionsBlacklist")
            if ctx.channel.id in blacklisted_channels:
                errors.append("- :no_entry: You are not allowed to use this command in this channel!")

        if errors:
            await ctx.message.reply("Couldn't add poll reactions:\n" + '\n'.join(errors))
            return

        try:
            # await ctx.message.reply("Adding emojis...")
            await message.add_reaction(upvote_emoji)
            if neutral_emoji:
                await message.add_reaction(neutral_emoji)
            await message.add_reaction(downvote_emoji)
        except revolt.errors.Forbidden:
            await ctx.message.reply(content=":no_entry: Couldn't add all poll reactions: Forbidden (maybe the user you're trying to add reactions to has blocked Rina)")
        # except revolt.errors.: # I'm suspecing KeyError
        #     await ctx.message.reply(content=":no_entry: Couldn't add all poll reactions: (at least) one of the emojis was not a real emoji!")
