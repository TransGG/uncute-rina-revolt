...
class CustomVcs(commands.Cog):
    ...
    async def editguildinfo(self, ctx: commands.Context, mode: str, option: str, value: str = None):
        ...
        --------------
        mode: :class:`str`
            Whether to 'view' or 'edit' guild settings
        option: :class:`str`
            Which option to view or edit. Usually a number, 01, 02,; 11, 12,; 21, 22,; etc.
        value: :class:`str`
            The value to change the option to. Is redundant if you're only viewing.
        """
        ...
        if option not in options:
            await ctx.send("Your mode has to be a number. Type 01, 02, 03, or 04 to find out what options you have:\n"
                           "- 01 : Help: Main server settings\n"
                           "- 02 : Help: Custom Voice Channels\n"
                           "- 03 : Help: Starboard settings\n"
                           "- 04 : Help: Bumping-related settings")
        ...
        if option.startswith("0"):
            if option == "01":
                await ctx.send("Main server settings:\n" +
                ...
                await ctx.send("Custom Voice Channels:\n" +
                               ...
                               "`23`: VC no-mic (Which channel should custom new)")
            elif option == "03":
                await ctx.send("Starboard settings:\n" +
                               ...
                               "`35`: Starboard downvote initiation value (how many total (up/down)votes must a message have before deleting it if its score is below 0?)")
            elif option == "04":
                await ctx.send("Bumping-related settings:\n" +
        ...
        if mode == 2:
            ...
            try:
                if option == "11":
                    value = await validate(value, "You have to give an ID for the text channel you want to use.")
                    if value is None:
                        return
                    ch = ctx.client.get_channel(value)
                    if type(ch) is not revolt.TextChannel:
                        await ctx.send(f"The ID you gave wasn't for the type of channel I was looking for! (Need <class 'revolt.TextChannel'>, got {type(ch)}).")
                        return
                    ...
                    for channel_id in channel_ids:
                        channel = await validate(channel_id.strip(), "You need to give a list of ids, separated by a comma, for this new blacklist.")
                        if channel is None:
                            return
                        blacklist.append(channel)
                    collection.update_one(query, {"$set": {options[option]: blacklist}}, upsert=True)
                elif option == "21":
                    value = await validate(value, "You have to give an ID for the voice channel you want to use.")
                    if value is None:
                        return
                    ch = ctx.client.get_channel(value)
                    if type(ch) is not revolt.VoiceChannel:
                        await ctx.send(f"The ID you gave wasn't for the type of channel I was looking for! (Need <class 'revolt.VoiceChannel'>, got {type(ch)})")
                        return
                    collection.update_one(query, {"$set": {options[option]: ch.id}}, upsert=True)
                elif option == "22":
                    value = await validate(value, "You have to give an ID for the category you want to use.")
                    if value is None:
                        return
                    ch = ctx.client.get_channel(value)
                    if type(ch) is not revolt.Category: #TODO:, note: I haven't been able to copy the category id yet lol
                        await ctx.send(f"The ID you gave wasn't for the type of channel I was looking for! (Need <class 'revolt.Category'>, got {type(ch)})")
                        return
                    collection.update_one(query, {"$set": {options[option]: ch.id}}, upsert=True)
                elif option == "23":
                    value = await validate(value, "You have to give an ID for the text channel you want to use.")
                    ...
                    value = await validate(value, "You have to give a numerical ID for the emoji you want to use.")
                    if value is None:
                        return
                    emoji = await ctx.client.fetch_emoji(value)
                    if type(emoji) is not revolt.Emoji:
                        await ctx.send(f"The ID you gave wasn't an emoji! (i think) (or not one I can use)")
                        return
                    ...
                    try:
                        ...
                        value = None
                    value = await validate(value, "You need to give an integer value for your new minimum amount.")
                    if value is None:
                        return
                    if value < 1:
                        await ctx.send("Your new value has to be at least 1!")
                    ...
                    value = await validate(value, "You need to give a numerical ID for the bot id you want to use.")
                    if value is None:
                        return
                    user = ctx.client.get_user(value)
                    if not isinstance(user, discord.User):
                        await ctx.send("The ID you gave wasn't for a valid user!")
                        return
                    collection.update_one(query, {"$set": {options[option]: user.id}}, upsert=True)
                elif option == "42":
                    value = await validate(value, "You need to give a numerical ID for the channel you want to use.")
                    if value is None:
                        return
                    ch = ctx.client.get_channel(value)
                    if not isinstance(ch, revolt.Messageable):
                        await ctx.send("The ID you gave wasn't for the type of channel I was looking for!")
                        return
                    collection.update_one(query, {"$set": {options[option]: ch.id}}, upsert=True)
                elif option == "43":
                    value = await validate(value, "You need to give a numerical ID for the role you want to use.")
                    if value is None:
                        return
                    ch = ctx.server.get_role(value)
                    if not isinstance(ch, discord.Role):
                        await ctx.send("The ID you gave wasn't a role!")
                        return
                    collection.update_one(query, {"$set": {options[option]: ch.id}}, upsert=True)
